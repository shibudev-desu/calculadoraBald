...
def calcular(display): 
    ...
    try: 
        ...
        expressao = display.get().replace("×", "*").replace("÷", "/").replace("√", "math.sqrt").replace("sin(", "math.sin(math.radians(").replace("cos(", "math.cos(math.radians(").replace("tan(", "math.tan(math.radians(").replace("log(", "math.log10(").replace("ln(", "math.log(").replace(",", ".").replace("^", "**") 
...
class Normal(tk.Frame): 
    ...
    def __init__(self, parent, controller): 
        ...
        botoes = [ 
            ...
            [("0", lambda: inserir("0", self.display)), (",", lambda: inserir(",", self.display)), ("=", lambda: calcular(self.display))] 
        ] 
...
class Cientifica(tk.Frame): 
    ...
    def __init__(self, parent, controller): 
        ...
        botoes = [ 
            [("", "SHIFT", toggle_shift, 7), ("", "ALPHA", nao_implementado, 7),  ("", "REPLAY", nao_implementado, 7), ("CLR", "MODE", nao_implementado, 7), ("", "ON", lambda: limpar_tudo(self.display),7)], 
            [("x!", "x⁻¹", um_sobre_x,12), ("nPr", "nCr", nao_implementado, 12),("   ", "    ", nao_implementado, 12), ("Rec( :", "Pol(", nao_implementado, 12), ("³√", "x³", ao_cubo,12)], 
            [("d/c", "ab/c", nao_implementado, 7), ("", "√", calcular_raiz,7), ("", "x²", ao_quadrado,7), ("x√", "^", lambda: inserir("^", self.display),7), ("10^", "log", lambda: inserir("log(", self.display),7), ("e^ e", "ln", lambda: inserir("ln(", self.display),7)], 
            [("A", "(-)", nao_implementado, 7), ("⭠ B", ".,, ,,", nao_implementado, 7), ("hyp", "C", nao_implementado, 7), ("sin⁻¹   D", "sin", lambda: inserir("sin(", self.display),7), ("cos⁻¹ E", "cos", lambda: inserir("cos(", self.display),7), ("tan⁻¹ F", "tan", lambda: inserir("tan(", self.display),7)], 
            [("STO", "RCL", nao_implementado, 7), ("⭠", "ENG", nao_implementado, 7), ("", "(", lambda: inserir("(", self.display),7), ("x", ")", lambda: inserir(")", self.display),7), (":     Y", ",", nao_implementado, 7), ("M- M", "M+", nao_implementado, 7)], 
            [("", "7", lambda: inserir("7", self.display),7), ("", "8", lambda: inserir("8", self.display),7), ("", "9", lambda: inserir("9", self.display),7),  ("INS", "DEL", nao_implementado, 7), ("OFF", "AC", nao_implementado, 7)], 
            [("", "4", lambda: inserir("4", self.display),7), ("", "5", lambda: inserir("5", self.display),7), ("", "6", lambda: inserir("6", self.display),7), ("", "x", lambda: inserir("x", self.display),7), ("", "÷", lambda: inserir("÷", self.display),7)], 
            [("S-SUM", "1", lambda: inserir("1", self.display),7), ("SVAR", "2", lambda: inserir("2", self.display),7), ("", "3", lambda: inserir("3", self.display),7), ("", "+", lambda: inserir("+", self.display),7), ("", "-", lambda: inserir("-", self.display),7)], 
            [("Rnd", "0", lambda: inserir("0", self.display),7), ("Ran#", ".", lambda: inserir(".", self.display),7),  ("π", "EXP", nao_implementado, 7), ("DRG+", "Ans", nao_implementado, 7), ("%", "=", lambda: calcular(self.display),7)] 
        ] 
